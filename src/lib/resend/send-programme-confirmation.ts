// P-00c-ENROL — webhook-safe boundary for the guest programme-enrolment email.
//
// The Stripe webhook (payment_intent.succeeded) MUST always return 200 once the
// signature passes, so a failed email can never throw out of the handler. The
// template sender in ./emails throws on Resend failure; this wrapper catches it,
// logs, and returns a boolean the caller can log without branching its control flow.

import {
  sendGuestProgrammeConfirmation,
  type GuestProgrammeConfirmationParams,
} from './emails'

/**
 * Sends the guest programme-enrolment confirmation email. Never throws.
 * @returns true if Resend accepted the send, false on any failure (already logged).
 */
export async function sendProgrammeConfirmation(
  params: GuestProgrammeConfirmationParams,
): Promise<boolean> {
  try {
    await sendGuestProgrammeConfirmation(params)
    return true
  } catch (err) {
    console.error(
      `[sendProgrammeConfirmation] failed for enrolment ${params.enrolmentReference}:`,
      err instanceof Error ? err.message : err,
    )
    return false
  }
}
