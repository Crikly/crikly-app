...
// P-00c-ENROL — guest programme-enrolment checkout. A programme-shaped copy of
// GuestBookingFlow (do NOT modify that file): same "Your details" form, Stripe
// PaymentElement + ExpressCheckoutElement, and confirmation screen; differs in
...
interface GuestEnrolmentFlowProps {
  ...
  /** Session UUIDs being paid for (per_session only — empty for block_upfront). */
...
/**
 * Outer orchestrator. Owns the checkout ↔ confirmed view switch. The checkout
 * view is wrapped in <Elements> (deferred-intent mode) so the inner form can use
 * the Stripe hooks.
 */
export function GuestEnrolmentFlow({
  coachId,
  programmeId,
  paymentType,
  selectedSessionIds,
  ...
  const programmeHref = `/coaches/${coachId}/programmes/${programmeId}`
  ...
  async function handleCopyReference(): Promise<void> {
    if (!confirmed) return
    try {
      await navigator.clipboard?.writeText(confirmed.enrolmentReference)
      setCopied(true)
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: 'Crikly enrolment confirmed',
          text: `My Crikly programme enrolment is confirmed — reference ${confirmed.enrolmentReference}.`,
          url: programmeHref,
        })
      } else {
        ...
        <div className="mt-7 flex w-full items-center justify-between gap-3 rounded-[12px] bg-[#F0F7FF] px-4 py-3.5 lg:px-5 lg:py-4">
          ...
          <div className="flex flex-shrink-0 items-center gap-2">
            <button
              ...
              aria-label="Copy enrolment reference"
...
interface GuestEnrolmentFormProps {
  coachId: string
  programmeId: string
  paymentType: 'per_session' | 'block_upfront'
  selectedSessionIds: string[]
...
function GuestEnrolmentForm({
  ...
  /**
   * Create the enrolment + PaymentIntent server-side. Returns the client secret
   * and enrolment reference, or sets an error and returns null.
   */
  ...
  async function handlePay(): Promise<void> {
    ...
    const created = await createEnrolment({
    ...
    onConfirmed(created.enrolmentReference, form.email)
  }
  ...
  async function handleExpressConfirm(event: StripeExpressCheckoutElementConfirmEvent): Promise<void> {
    ...
    onConfirmed(created.enrolmentReference, billing?.email || form.email)
  }
  ...
  const errorBanner = error ? (
    <div role="alert" className="flex items-start gap-2.5 rounded-[10px] bg-danger/10 p-3.5 text-danger">
      ...
      <div className="min-w-0 text-sm">
        {error === 'spots_taken' ? (
          <>
            <p className="font-medium">This programme just filled up.</p>
  ...
  return (
    <div>
      ...
      <Link
        href={programmeHref}
        className="inline-flex items-center gap-1.5 text-[14px] font-medium text-slate-500 transition-colors hover:text-neutral-900"
      >
        <ArrowLeft size={16} aria-hidden="true" />
        Back to programme
      </Link>
      ...
      <p className="mb-6 mt-2 text-base text-slate-500 lg:mb-7">
        {`Review your enrolment with ${summary.coachName} and pay securely. You won't be charged until you confirm.`}
      </p>
        ...
        <div className="flex flex-col gap-6 lg:col-start-1 lg:row-start-1 lg:row-span-2 lg:gap-5">
          ...
          <section className="flex flex-col">
            <div className="flex flex-col gap-3 lg:gap-4 lg:rounded-[12px] lg:border lg:border-neutral-100 lg:bg-white lg:p-6">
              ...
              {/* Express checkout — real Apple Pay / Google Pay only. */}
              <div>
                <ExpressCheckoutElement
                ...
                {/* Static placeholders — shown only while no real wallet is available. */}
                {!walletsAvailable && (
              ...
              <div className="flex flex-col gap-2.5">
                ...
                {billingSame ? (
                  <p className="pl-[30px] text-[13px] leading-[1.45] text-slate-500">
                    {billingSummary}
                  </p>
                ) : (
                  <div className="flex flex-col gap-2.5">
