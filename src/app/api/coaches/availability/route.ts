...
export async function POST(
  ...
  try {
    ...
    // BUG-17: an ad-hoc block is bound to a single calendar date, so specific_date
    // is mandatory — the date-scoped conflict guard below cannot run without it.
    if (body.is_recurring === false && !body.specific_date) {
      validationErrors.push('specific_date is required for ad-hoc (non-recurring) blocks')
    }
    ...
    if (body.is_recurring === false) {
      ...
      // commitment set on that calendar date — recurring availability, other
      // ad-hoc slots, programme sessions, and confirmed 1-on-1 bookings.
      // getCoachCommitments enforces ownership via coach_profile_id, so the admin
      // client (no dev-DB auth_user_id mismatch) is safe. specific_date is
      // guaranteed present by validation above.
      const adminSupabase = createAdminClient()
      const commitments = await getCoachCommitments(adminSupabase, coachProfile.id, body.specific_date)
      ...
      // weekday (Decision B — recurring creation is NOT blocked against programmes
      // or bookings; BUG-16's read-side suppression hides any programme collision
      // on the public calendar). Only OTHER recurring blocks are compared — ad-hoc
      // rows share the weekday index but belong to a single date and must not veto
      // a weekly pattern.
      ...
      for (const existing of potentialConflicts ?? []) {
        ...
        if (newStart < existingEnd && newEnd > existingStart) {
          return NextResponse.json(
            {
              error: 'Conflict detected',
              message: `You already have a recurring block on ${dayNames[body.day_of_week]} from ${existing.start_time.slice(0, 5)}–${existing.end_time.slice(0, 5)}. Adjust the time or day.`,
