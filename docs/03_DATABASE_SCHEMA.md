...
| additional_participants | jsonb | YES | null | Group-booking players 2..N (migration 054, P-10): `[{name text, age int\|null, child_profile_id uuid\|null}]`. Primary player stays in participant_name/participant_age (back-compat). child_profile_id links a parent's child for authed bookings, null for guest players — deliberately no FK (point-in-time record, survives profile removal; coach_time_claims.source_id precedent). Child session-history queries also match on it via jsonb containment. CHECK: array type only; entry rules (names required, ages 1–99 optional, max entries = highest group_price_tiers key − 1 counting the primary) live in the shared app-side participants validator used by both booking routes (migration-053/AF-H-56 pattern). NULL = 1-on-1 booking. |
| deleted_at | timestamptz | YES | null | Soft delete |
| created_at | timestamptz | NO | now() | |
| updated_at | timestamptz | NO | now() | |
...
**RLS Policies:**
- SELECT: Own enrolments (parent/player who enrolled) — predicate fixed in migration 055 (EXISTS through `user_profiles.auth_user_id`; the original compared `booked_by_user_id` to `auth.uid()` and never matched). NOTE: `_update_own` still carries the broken predicate — intentionally, until the enrolment-cancel work decides parent-side UPDATE access.
...
**Function:** `link_provisional_bookings(p_target_profile_id uuid, p_provisional_profile_ids uuid[]) RETURNS jsonb` (migration 052; SECURITY DEFINER, `search_path=public`, service_role only — EXECUTE revoked from PUBLIC/anon/authenticated). P-04-B guest→account linking: atomically re-points `bookings.booked_by_user_id` and `group_programme_enrolments.booked_by_user_id` from live provisional guest profiles (is_provisional, auth_user_id NULL, not deleted — row-locked FOR UPDATE against concurrent double-link) to the caller-supplied real profile, then soft-deletes the emptied provisional rows. Money columns never touched. Arguments are derived server-side by `POST /api/auth/link-guest-bookings` from the verified session email (matched via Stripe PI metadata `guest_email`, cross-checked against `payment_intents`) — nothing client-supplied reaches the function. Returns `{bookings, enrolments, profiles}` counts.
