...
| BUG-QA-02 | Calendar week-strip day indicators — per-day dots now use amber (pending wins), teal-600 (confirmed/completed), brand-600 fallback; weekOffset query-param support on bookings API (clamped ±52); AbortController stale-response cancellation; clears state before new fetch. | @FrontendDeveloper | 🟢 | develop | ✅ |
