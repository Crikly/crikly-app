...
| BUG-STRIPE-ONBOARDING-COMPLETE-WIRING | Wire Stripe webhook (`account.updated` event) to set `coach_profiles.stripe_onboarding_complete = (account.charges_enabled && account.payouts_enabled)`. Column exists since migration 002 but nothing currently writes `true` to it. Unlocks dashboard's basic-profile completion check switching from `!!stripe_account_id` (over-reports) to the real charges+payouts gate. | @PaymentsEngineer | 🟡 | TBD | ⚪ |
