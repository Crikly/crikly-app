...
**Changed:** Added CF-TOKEN-AUDIT (visual design-token audit — replace hardcoded hex with Tailwind tokens + suppress dark-mode across 11 coach files).
...
| API-COACH-REVIEWS-REAL-WIRING | Replace STUB in `/api/coaches/reviews` with a real Supabase query — nested `reviews + coach_replies(reply_text)` filtered by coach_profile_id + is_visible, ordered newest-first. Reads rating_avg + rating_count from coach_profiles denorm cache. Also regenerates `src/types/database.ts` to include migration 029 changes. Follow-ups: API-COACH-REVIEWS-RATING-TREND (real 30d delta), API-COACH-REVIEWS-PAGINATION, BUG-REVIEWS-REPLY-PERSIST, OPP-REVIEW-COMMENT-NULLABLE. | @BackendDeveloper | 🟡 | fix/reviews-api-wiring | ✅ |
| DS-RIGHT-PANEL-01 | Universal right-panel command-centre — replaces 7 route-conditional variants with a single layout rendered on every coach route. 6 sections: This Week, Daily Lineup (with session-detail popup), Action Needed, This Week's Pulse, Next Payout (STUB empty state), Your Profile (STUB views). Ungated BookingsProvider (now mounts on every coach route). Added `?tab=week` to bookings API + `thisWeek` field to BookingsContext. Deleted duplicate ThisWeekStrip/TodayLineup decls from CoachHomeClient + dashboardData prop pattern. ~700 LOC deleted, ~550 added. Follow-ups: API-CMD-CENTRE, API-RIGHT-PANEL-PAYOUT-REAL, BUG-RIGHT-PANEL-UNANSWERED-REVIEWS, BUG-RIGHT-PANEL-PROFILE-VIEWS, BUG-PROGRAMME-CREATE-PREVIEW-LOST, PERF-RIGHT-PANEL-DASHBOARD-DEDUPE, PERF-RIGHT-PANEL-WEEK-CAP. | @FrontendDeveloper | 🟡 | feat/right-panel-redesign | ✅ |
| API-CMD-CENTRE | Aggregation endpoint for right-panel non-session data: real profile completeness (full 6-check via DB counts on coach_sports/qualifications/availability_templates/stripe), all-time week stats (sessions, revenue, completion %), unanswered reviews count. Replaces Phase-1 STUBs in CoachRightPanel.tsx. | @BackendDeveloper | 🟢 | TBD | ⚪ |
| API-RIGHT-PANEL-PAYOUT-REAL | Wire Section 5 next-payout card to real Stripe payout schedule + aggregated completed-session totals. Phase 1 always renders the empty state ("No pending payout"). | @PaymentsEngineer | 🟡 | TBD | ⚪ |
| BUG-RIGHT-PANEL-UNANSWERED-REVIEWS | Expose count of reviews without `coach_replies` rows per coach. Wires Section 3 second action row (currently hidden via STUB 0). | @BackendDeveloper | 🟢 | TBD | ⚪ |
| BUG-RIGHT-PANEL-PROFILE-VIEWS | Build view-tracking infrastructure (events table + endpoint) so Section 6 can show real "X views last 7 days" stat. Phase 1 always shows "No views yet". | @BackendDeveloper | 🟡 | TBD | ⚪ |
| BUG-PROGRAMME-CREATE-PREVIEW-LOST | Move programme-create live preview into the create page itself (e.g., bottom strip on mobile, fixed lower-right card on desktop). Previously rendered via CoachRightPanel route conditional, removed in DS-RIGHT-PANEL-01. | @FrontendDeveloper | 🟢 | TBD | ⚪ |
| PERF-RIGHT-PANEL-DASHBOARD-DEDUPE | Universal right panel fires 4 BookingsContext fetches client-side on /coach/dashboard, partially overlapping the 11 server-side dashboard queries. Reconcile via shared SSR-pre-hydrated context or remove dashboard-specific server queries. | @FrontendArchitect | 🟢 | TBD | ⚪ |
| PERF-RIGHT-PANEL-WEEK-CAP | `?tab=week` returns max 10 rows (PAGE_SIZE). Coaches with >10 sessions in a single week see truncated dots + lineups. Either bump cap for week tab or paginate. | @BackendDeveloper | 🟢 | TBD | ⚪ |
| BUG-COMPLETION-ALIGNMENT | Align profile-completion checks across 4 surfaces (4 bugs in one commit). FIX 1: right panel Stripe check now uses `chargesEnabled && payoutsEnabled` (cache-first, matches ProfileEdit). Dashboard keeps `!!stripe_account_id` for now — server component can't read cache; properly fixed via BUG-STRIPE-ONBOARDING-COMPLETE-WIRING. FIX 2: removed `years_experience` from right panel (no other surface used it); now 3 real + 3 STUB checks. FIX 3: renamed dashboard label "Sport & pricing" → "Sport" (the check only verifies sports presence, not pricing). FIX 4: dashboard basic-profile check now requires `avatar_url`; onboarding ProfileStep removed `gender` from its 4-field meter (PRD REQ-C-028 — photo required, gender not). | @FrontendDeveloper | 🟡 | fix/completion-alignment | ✅ |
| BUG-STRIPE-ONBOARDING-COMPLETE-WIRING | Wire Stripe `account.updated` webhook to persist `coach_profiles.stripe_onboarding_complete`. Created `src/app/api/webhooks/stripe/route.ts` from scratch — signature-verified via `STRIPE_WEBHOOK_SECRET`, raw body via `request.text()`, `runtime='nodejs'`, `dynamic='force-dynamic'`. `account.updated` handler computes `isComplete = charges_enabled && payouts_enabled` and writes to `coach_profiles` via adminSupabase. Both directions handled; DB errors swallowed to prevent retry loops. Stripe Dashboard endpoint registered (Connected accounts, account.updated only). Tested locally with Stripe CLI — [200] confirmed. `STRIPE_WEBHOOK_SECRET` added to Vercel. `docs/04_API_REFERENCE.md` updated with new Webhook Routes section + stale placeholder collapsed. Shipped as d74bb96. | @PaymentsEngineer | 🔴 | develop | ✅ |
| API-CMD-CENTRE | New `GET /api/coaches/profile-completeness` endpoint exposes the 3 server-side completion booleans (sports / qualifications / availability) + unanswered reviews count to the right-panel command-centre. Mirrors the dashboard's existing inline count queries. Also extends `GET /api/coaches/reviews` with an `unanswered_count` field computed from the in-memory array. Right panel: STUB `count += 3` replaced with real reads; `unansweredReviewsCount = 0` STUB replaced with real count from the new endpoint — Section 3 "Unanswered reviews" row now appears when there are replies pending. 60s sessionStorage TTL cache mirrors the Stripe pattern. Closes BUG-RIGHT-PANEL-UNANSWERED-REVIEWS. | @BackendDeveloper | 🟡 | fix/cmd-centre | ✅ |
| BUG-DASHBOARD-PROGRAMMES-NOT-SHOWING + PERF-PROGRAMMES-LOAD + BUG-PROGRAMME-CREATE-PREVIEW-LOST | 3 programmes bugs in one commit. FIX 1: dashboard query now includes drafts (`.in('status', ['active','draft'])`) + handles rolling programmes with null `ends_at` (`.or('ends_at.gte.<now>,ends_at.is.null')`). GroupCard renders drafts with opacity-60 + "Draft" badge + "Not published yet" sub-text. FIX 2: 30s sessionStorage cache on ProgrammesManagement's list fetch (existing API is NOT N+1 — Fix-65-1 already batched sports lookup). Cache invalidated before refetch in handlePublish / handleCancel / handleDelete. FIX 3: deleted dead CustomEvent dispatch + ProgrammePreviewEventDetail interface (consumer was deleted in DS-RIGHT-PANEL-01). New inline `ProgrammeCreatePreview` component reads form/sports/step directly, renders as sticky right-column panel on xl+, hidden on mobile. Closes BUG-PROGRAMME-CREATE-PREVIEW-LOST. | @FrontendDeveloper | 🟡 | fix/programmes-fixes | ✅ |
| BUG-UPNEXT-PROGRAMME-SESSIONS | Dashboard "Up next" now considers active group programmes alongside 1-to-1 bookings. Programme sessions are derived from `days_of_week + start_time` (no row in bookings) — new `nextProgrammeOccurrence` helper projects forward up to 14 days for each programme, respects `ends_at`, picks the soonest valid slot. Compared against the existing upNextBooking and the soonest wins (tie → booking). `upNextSession` shape extended with `sessionType: '1-to-1' \| 'group'` + optional `groupProgrammeId`. CoachHomeClient adds a session-type badge next to the "Starts in Xm" pill on the hero card. Dashboard programmes query extended to also select `days_of_week, start_time, duration_minutes, ends_at, venue_name` — no API change, no DB change. | @BackendDeveloper | 🟡 | fix/upnext-programmes | ✅ |
| CF-PROGRAMMES-IMAGE-PICKER | Coaches can now add a cover photo to each programme — either pick from 6 curated Unsplash images for their sport, or upload their own to Supabase Storage. Migration 030 adds nullable `image_url` to `group_programmes`. New `src/lib/programme-images.ts` exports `SPORT_IMAGES` keyed on lowercase sport name (Cricket only Phase 1, with Cricket fallback). New `<ProgrammeImagePicker>` shared component with two tabs ("Choose a photo" + "Upload your own"); upload tab degrades gracefully when the Storage bucket isn't yet provisioned. Wired into Create + Edit. POST + PATCH API routes accept `image_url` (null clears). Dashboard `GroupCard` uses each programme's real image with `DEFAULT_PROGRAMME_IMAGE` fallback — `group1Url`/`group2Url` alternating placeholder removed. Follow-ups: INFRA-PROGRAMME-IMAGES-BUCKET (manual create + RLS policies), INFRA-PROGRAMME-IMAGES-CLEANUP (storage orphans on programme delete/replace), OPP-PROGRAMMES-CARD-IMAGE-HEADER (render image on /coach/programmes list cards — data wired but not displayed). | @FrontendDeveloper | 🟡 | feat/programmes-image-picker | ✅ |
| CF-PROG-AGE-GROUP | Age group multi-select on Create Programme Step 1 + Edit. New `src/components/coach/programmeConstants.ts` exports the `PROGRAMME_AGE_GROUPS` allowlist (`Under 8`, `Under 10`, `Under 13`, `Under 16`, `Adults (16+)`, `All ages`). "All ages" is mutually exclusive with specific groups (UI-enforced XOR; min 1 required). PillButton group on Step 1; consumed downstream by the programme grid card chip row (UI-PROG-GRID) and the detail-modal age-groups section. POST + PATCH validate against the allowlist and write `group_programmes.age_groups text[]`. EditProgramme filters legacy rows where the DB default `'{}'` left the array empty, falling back to `['All ages']`. Shipped as c94c4a0. | @FrontendDeveloper + @BackendDeveloper | 🟡 | develop | ✅ |
| CF-PROG-START-DATE | Commencing date picker on Create Programme Step 2 (Fixed flow) + first-session anchor wired into the share card preview. Form gains `starts_at: 'YYYY-MM-DD'` populated by a DatePicker (Fixed) or by the rolling start-date control (Rolling). All date math uses LOCAL components rather than `toISOString().split('T')[0]` — fixes a BST off-by-one where users in positive-UTC saw their pattern shift back one calendar day. POST + PATCH derive `group_programmes.starts_at` / `ends_at` from these inputs; list endpoint exposes `starts_at` only (not `ends_at`). EditProgramme converts the timestamptz back to `YYYY-MM-DD` on load. Shipped as 91f7f47. | @FrontendDeveloper + @BackendDeveloper | 🟡 | develop | ✅ |
| CF-PROG-SESSION-LIST | Full SessionCalendar redesign (`src/components/coach/SessionCalendar.tsx`). Coaches now see and edit the canonical session list as a 6×7 month calendar (Monday-first, UK). Form gains `session_dates: SessionEntry[]` (`{ date, startTime, endTime, slots? }`) emitted via `onChange`. Supports Fixed (count-based pattern), Rolling (date-range pattern), and Camp (multi-slot days, `slots[]`) flows. Tap a session dot to edit time / remove; tap a non-pattern weekday to also add it to the days pill row (auto-add via `onSelectedDaysAdd`). Skip/restore section for unwanted pattern dates. Out-of-month overflow days render lighter (later refined under CF-PROG-UX-BATCH-MAY-2026 / Fix-56). End-time field replaces the Duration pill row — server derives `duration_minutes` at PATCH time. Same BST-safe local-date construction throughout. Shipped as d4bc126. | @FrontendDeveloper + @BackendDeveloper | 🟡 | develop | ✅ |
| CF-PROG-SHARE-CARD | Shareable programme card — coach can show a parent the programme at a glance, send the canonical link via a WhatsApp deep link, or download a PNG. New `src/components/coach/ShareCard.tsx` (inline styles only — html-to-image canvas capture doesn't reliably inline Tailwind) and `src/components/coach/ShareCardModal.tsx` (WhatsApp button + PNG download via html-to-image; lazy coach-meta fetch with 5-min sessionStorage cache). Initial commit d8f3494. Subsequent modal-position and brand-mark refinements landed under CF-PROG-UX-BATCH-MAY-2026 (Fix-58 logo swap; modal-centering follow-up 5ae75ae). | @FrontendDeveloper | 🟡 | develop | ✅ |
| UI-CARD-FIX | Dashboard "Your programmes" horizontal scroll polish (`src/components/coach/CoachHomeClient.tsx` GroupCard). `items-stretch` on the row + `h-full` on the card root + `line-clamp-2` on titles so cards in the same row match height regardless of title length. Right-edge fade gradient (white→transparent, `w-8`, `from-white/80`) signals "more to scroll"; left-edge gradient appears conditionally once `scrollLeft > 10` so coaches know they have scrolled. Scroll position tracked via `useRef` + a passive scroll listener. Initial commit 51de64b; gradient-refinement follow-up shipped as cdc1bcf. | @FrontendDeveloper | 🟢 | develop | ✅ |
| UI-PROG-GRID | ProgrammesManagement screen rebuilt as a 2-column grid (`grid grid-cols-2 gap-3 p-4`). Card shape: 120px cover image (or brand-gradient + `ImageIcon` fallback when `image_url` is null), absolutely-positioned status pill in light-bg + dark-text colours (Active / Full / Draft) per design, title + sport pill row, age-group chips with `+N` overflow, schedule + starting-date + venue meta rows, colour-coded fill bar (brand <90%, amber ≥90%, success at 100%), action pill row. Card root `flex flex-col h-full`; content area `flex-1` with a `flex-1` spacer so the price + fill-bar bottom-align across the row. Tab count chips adopted; active-tab label `text-neutral-900 font-bold` + 2px brand-600 underline (single indicator, not double). Hover: `scale-[1.015] -translate-y-0.5 shadow-lg` over 200ms ease-out. All action-row handlers preserved (Publish / Edit / Share / Manage / Cancel + confirm-states). Initial commit 4734cce. Subsequent Promote-button removal (0b9f43b) and divider-alignment polish (b05e116, 654ce56) landed under the same task ID. | @FrontendDeveloper | 🟡 | develop | ✅ |
| UI-DATE-PICKER | Replaced all 14 native `<input type="date">` and `<input type="time">` controls across 5 files with custom `src/components/ui/DatePicker.tsx` and `src/components/ui/TimePicker.tsx`. DatePicker: 280px popover, 6×7 grid Monday-first, today dot, past-date disable when `minDate` set, click-outside / Esc close, BST-safe `toLocalDateString` helper (the prior `new Date().toISOString().split('T')[0]` pattern shifted dates by one day in BST). TimePicker: 160px popover, HR/MIN columns, auto-scroll selected row to centre on open, off-step minute values preserved (injected into list so existing data isn't silently mutated). New shared `todayYYYYMMDD()` exported from DatePicker is now the single BST-safe source of `min`-date strings used by CreateProgramme, EditProgramme, Schedule, and SessionCalendar. The previously-uncontrolled Schedule `defaultValue` stub gained its own `useState`. Shipped as 904f547. | @FrontendDeveloper | 🟡 | develop | ✅ |
| CF-PROG-EDIT-PARITY | EditProgramme Step 2 schedule section brought into parity with CreateProgramme. Adds `end_time` (TimePicker replaces the Duration pill group), Commencing date (Fixed) / Rolling start+end date (Rolling) DatePickers above the time row, and `onSelectedDaysAdd` on the embedded SessionCalendar so tapping a non-pattern date auto-adds the weekday to the pill row. `form.duration_minutes` retained as a dead form field; PATCH derives it from `diffMinutes(start_time, end_time)` (matches Create). `rolling_end_date` seeded from `data.ends_at` on load and round-trips through PATCH. SessionCalendar's `defaultEndTime` now reads `form.end_time` directly (the prior IIFE that recomputed from duration is removed). 18px / 140px legacy Start-time chrome standardised to the 44px DatePicker pattern. Shipped as ba4e544. | @FrontendDeveloper | 🟡 | develop | ✅ |
| CF-PROG-UX-BATCH-MAY-2026 | Batch UX polish for the programmes module — eight independent items shipped together as commit 342c284 (originally labelled Fix-54..Fix-62, consolidated under this Task ID because Fix-54..Fix-56 collided with pre-existing CF-D bookings entries in section 3D). Covers: PillButton layout-shift on selection (Check icon removed — brand-bg + white text already signals selection); ProgrammeImagePicker variety (random Unsplash page 1–10 + per-page sessionStorage cache + 30-min TTL; Refresh now re-fetches a new page rather than re-slicing); out-of-month session dots in SessionCalendar render `bg-brand-100 text-brand-600` (lighter) rather than full `brand-600`; Step 4 CTA order swap (Publish now → top primary, Save as draft → secondary outline); ShareCard wordmark replaced with `<img src="/logo.png">` whitened via `filter: brightness(0) invert(1)` for canvas-stable PNG output; camp-mode flow (toggle moved above Days-of-week; hidden when `schedule_type === 'rolling'`; Start/End time row hidden when `campMode === true`; auto-reset `campMode: false` on switch to Rolling); rolling field order (Date range hoisted above the Start/End time row so the order matches Fixed). Sibling work tracked separately: UI-DATE-PICKER (date/time pickers platform-wide) and CF-PROG-EDIT-PARITY (Edit/Create parity) — both have their own rows immediately above. Shipped as 342c284. | @FrontendDeveloper | 🟡 | develop | ✅ |
| CF-PROG-SESSIONS-DB | Persist `session_dates` (group_programme_sessions rows) and `camp_mode` (group_programmes column) to the database. Migration 031 adds `group_programmes.camp_mode boolean NOT NULL DEFAULT false`, `group_programme_sessions.slots jsonb NULL` (CHECK array-only), and `UNIQUE (group_programme_id, session_date)` for safe UPSERT. POST handler validates per-entry session shape (HH:MM + YYYY-MM-DD), inserts the programme then bulk-inserts session rows, soft-deletes the orphan programme on session-write failure for atomic semantics. PATCH handler reconciles sessions via in-memory set-diff → `.delete().in('id', toDeleteIds)` + `.upsert(rows, { onConflict: 'group_programme_id,session_date' })`; reconciliation is silently skipped (with a `console.info` audit log) when `current_spots > 0`. GET returns `session_dates: SessionEntry[]` and `camp_mode: boolean` on the single-programme endpoint (list endpoint unchanged to avoid N+1). EditProgramme hydrates `campMode` from snake_case `data.camp_mode` with a TODO-flagged transitional fallback. `src/types/database.ts` pre-empts the regen with `camp_mode` + `slots` shapes. No backfill — legacy programmes' next coach save repopulates session rows. Passed both `supabase-migration-reviewer` and `crikly-code-reviewer` (one 🟡 + nits fixed inline). | @BackendDeveloper + @DatabaseArchitect | 🔴 | develop | ✅ |
| INFRA-PROGRAMME-IMAGES-BUCKET | Create Supabase Storage bucket `programme-images` (public) + RLS policies allowing authenticated coaches to upload to paths prefixed with their own `coach_profile_id`. Bucket creation not in CF-PROGRAMMES-IMAGE-PICKER scope because Studio is read-only per working-ethics (and CLI bucket creation needs separate review). Until this lands, the upload tab in ProgrammeImagePicker degrades to "Upload unavailable" gracefully. programme-images bucket created manually 25 May 2026. | @DevOpsEngineer | 🟢 | manual | ✅ |
| SCHEDULE-PROG-SESSIONS | Group programme sessions now visible on the Schedule grid as purple EventBlocks. New `src/app/api/coaches/programme-sessions/route.ts` — GET endpoint returns the coach's active programme sessions in a date range (three explicit queries: programmes → sessions → venues, no nested joins). Schedule.tsx: new `ProgrammeSessionBlock` interface, `programmeSessions` state + forward-compat `programmeSessionsRefreshKey`, week-scoped fetch useEffect mirroring the bookings pattern, `handleProgrammeSessionClick`, z-[14] render layer. `SessionPopover` programme branch wired with real title/spots/time/venue — CTA navigates to `/coach/programmes/{id}/roster`. `docs/04_API_REFERENCE.md` updated. Shipped as 31bb922. | @BackendDeveloper + @FrontendDeveloper | 🟡 | develop | ✅ |
...
| C-27 | Interactive flow tests + CI workflow fix (TEST-E2E-02). 3 new tests: P1 T1.4 add+delete availability template (Sun 17:00-19:00), P3 T3.3 multi-step programme cancel + DB draft assert, P5 T5.3 Go Live modal + DB is_profile_live assert. CI fix: `npm run build` step added before Seed, `npm run start` replaces `npm run dev`, 120s webserver timeout, html reporter added (fixes "No files found" upload-artifact failure). `data-testid` regex hardened: `.replace(' ', '-')` → `.replace(/\s+/g, '-')`. All 18 P1-P6 tests pass locally. | @QAEngineer | 🟢 | develop | ✅ |
| BUG-QA-01 | Booking detail header — removed sticky 17px mini-title, added canonical 28px bold h1; added top quick-approve/decline row for pending_approval status with actionError display; hex sweep (#0077CC→brand-600, #0066AA→brand-700, #E6F3FB→brand-50, bg-gray-50→bg-neutral-50); removed font-sans + inline fontFamily override. | @FrontendDeveloper | 🟢 | develop | ✅ |
| BUG-QA-02 | Calendar week-strip day indicators — per-day dots now use amber (pending wins), teal-600 (confirmed/completed), brand-600 fallback; weekOffset query-param support on bookings API (clamped ±52); AbortController stale-response cancellation; clears state before new fetch. (Surface-level patch — superseded by BUG-QA-02 re-fix in commit with BUG-QA-04/02/06.) | @FrontendDeveloper | 🟢 | develop | ✅ |
...
| BUG-QA-04 | Sidebar live dot condition — flipped to status-indicator: no dot when draft, green pulse when live, amber pulse when live+paused. (Surface-level patch — superseded by BUG-QA-04 re-fix in commit with BUG-QA-04/02/06.) | @FrontendDeveloper | 🟢 | develop | ✅ |
| BUG-QA-05 | Schedule Day/Month toggle hidden — removed Day + Month buttons and wrapper; replaced with single non-interactive "Week" chip; week-nav arrows + Today button unchanged. | @FrontendDeveloper | 🟢 | develop | ✅ |
| BUG-QA-04 (re-fix) | Root-cause fix: clear all coach caches (`profile-completeness-cache`, `stripe-status-cache`, `crikly:coach-share-meta`, `crikly:programmes`) on logout via new `clearAllCoachCaches()` in `src/lib/auth-cleanup.ts`; called in both signOut paths in settings/page.tsx. Resolves stale cache state that caused sidebar live-dot and other indicators to show wrong state after sign-out + sign-in. | @FrontendDeveloper | 🟢 | develop | ✅ |
| BUG-QA-02 (re-fix) | Root-cause fix: programme sessions now fetched in parallel alongside bookings for the right-panel calendar week-strip (new `weekStripProgrammeSessions` state + AbortController'd fetch); merged into `sessionsByDay`; 3-way dot colour (pending > confirmed-booking > programme > fallback); `sport_id` added to `/api/coaches/programme-sessions` SELECT + response; programme sessions navigate to `/coach/programmes/{id}/roster`. Resolves calendar showing only bookings, not programme sessions. | @FrontendDeveloper | 🟢 | develop | ✅ |
| BUG-QA-06 | `profile-completeness-cache.ts` extracted from inline cache logic in CoachRightPanel.tsx (mirrors `stripe-status-cache.ts` pattern). `hover:bg-[#0066AA]` → `hover:bg-brand-700` cleanup in CoachRightPanel. Purple safelist tokens added to tailwind.config.js. `LineupItem` type added to discriminate booking vs programme sessions in right-panel popup CTA. TypeScript narrowing fix: `ctaHref` ternary guards on `session.programme_id` directly (TS narrows to string). | @FrontendDeveloper | 🟢 | develop | ✅ |
...
| ID | Task | Agent | Risk | Branch | Status |
|---|---|---|---|---|---|
| P-07 | Create CRUD /api/children — child profiles + Screens 07/08/09 + ChildSelector | @BackendDeveloper | 🟡 | feature/p-07-child-profiles | ✅ |
...
*Crikly Build Plan v3.6 — May 2026 — Sync: added CF-PROG sprint, UI-DATE-PICKER, CF-PROG-UX-BATCH-MAY-2026, CF-PROG-SESSIONS-DB rows; flipped INFRA-PROGRAMME-IMAGES-BUCKET to complete.*
