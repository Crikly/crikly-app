...
P-04-C: lightweight switch between roles the account ALREADY holds —
updates `user_profiles.active_role` only. None of the POST setup
side-effects run (no `user_roles` upsert, no `coach_profiles` creation,
no welcome email, no `user_metadata` rewrite). Used by the app shell's
role pill; adding a NEW role stays on POST.

**Auth:** Required (session). RLS-respecting cookie-scoped client.
...
**Response 200:**
```json
{ "success": true, "redirectTo": "/parent/dashboard" }
```

Errors: 400 `VALIDATION_ERROR` (unknown role), 401 `UNAUTHORIZED`,
403 `ROLE_NOT_HELD` (role not on the account — use POST to add it),
500 `UNKNOWN_ERROR`.
...
- `session_duration_minutes` (BUG-51) is the session length of THIS template's sport, resolved from `coach_sports`, or `null` for a sport-agnostic template (`sport_id` null) — the client then falls back to its default. Slot generation strides each block by its own value, so a multi-sport coach's blocks split correctly per sport
...
upcoming bookings → block on owed payouts → soft-delete `coach_profiles`
...
Four-case contract (BUG-49):
- **409 `UPCOMING_BOOKINGS`** — confirmed/pending_approval bookings dated today
  or later exist. "You have upcoming sessions. Please cancel them first."
- **409 `PENDING_PAYOUTS`** — payouts with status pending/processing/held
  exist. "You have pending payouts. Please wait for them to clear."
- **500 `BOOKING_HISTORY`** — the `auth.users` hard delete failed, typically
  the FK cascade hitting `bookings.coach_profile_id` ON DELETE RESTRICT (past
  bookings are financial records and must survive). Previously this was
  swallowed and the route returned a silent `{ success: true }` with the
  account still alive; now it surfaces and the client does NOT log the user
  out. Note: `coach_profiles` is already soft-deleted at this point — support
  (hello@crikly.app) resolves the remainder manually.
- **200 `{ success: true }`** — no upcoming bookings, no owed payouts, and
  every step (including the `auth.users` delete for only-coach accounts)
  genuinely succeeded.

**Error 500 (generic):** any pre-check read or the soft-delete write fails.
...
### GET /api/cron/process-coach-payouts
The coach payout transfer job (C-PAY-02). Moves money owed to coaches from the platform Stripe account to their Connect account once the booking is `completed` and `payout_eligible_at <= now()` (BR-03 clock, stamped by C-PAY-01).
**Status: Implemented — C-PAY-02**
**Auth: `Authorization: Bearer $CRON_SECRET`** — same pattern as the other crons; 401 otherwise, 500 if `CRON_SECRET` is unconfigured.

**Schedule:** `5 * * * *` (hourly at :05 — five minutes after C-PAY-01's completion pass) via `vercel.json`. Repo config only — activates when deployed with `CRON_SECRET` set (Lasith-owned). Local testing: `curl -H "Authorization: Bearer $CRON_SECRET" localhost:3000/api/cron/process-coach-payouts`.

**Selection (starvation-proof):** two queries, neither able to return settled work — (A) discovery: completed bookings with `payout_eligible_at <= now()` and NO payout row yet (PostgREST anti-join `payouts!left` + is-null); (B) work queue: payout rows `status IN ('pending','held') OR (failed AND retry_count < 5)` due by `scheduled_at`, booking embedded `!inner`. Each capped at 50, oldest first; merged and deduped (queue row wins).

**Amount (BR-02):** `coach_price_pence − balance_transaction.fee`, the ACTUAL Stripe fee expanded off the succeeded PaymentIntent's `latest_charge` — never estimated. If the fee ≥ coach price (unreachable at real prices) the row is skipped loudly with no payout row written — admin must resolve.

**Idempotency (one booking never receives two transfers):** payout row written BEFORE the transfer fires (write-intent-first); `payouts.booking_id` UNIQUE (migration 045) makes the row UUID stable per booking; that UUID is the `transfers.create` idempotency key; rows already `processing`/`paid` are skipped. A pre-existing row with no `stripe_transfer_id` (crashed prior run) is reconciled against `transfers.list({ transfer_group })` and adopts any existing transfer instead of re-firing — covers Stripe's ~24h idempotency-key expiry.

**Transfer:** `stripe.transfers.create` with `destination` = coach Connect account, `transfer_group` = `booking_reference`, `source_transaction` = the charge id (draws from the charge's balance, preventing insufficient-platform-balance failures), metadata `booking_id` + `payout_id`.

**Held contract (C-PAY-03, migration 043):** no `stripe_account_id` → `status='held'`/`no_stripe_account`; onboarding incomplete → `held`/`stripe_onboarding_incomplete`. A hold is not a failure (no `retry_count`, no `failure_reason`). Held rows re-evaluate every run at zero Stripe cost; release (held → pending, `held_reason` cleared in the same UPDATE) and transfer happen in the same run once Stripe is ready.

**Failure handling:** per-row isolation — a Stripe failure marks that row `failed` (`failure_reason`, `retry_count`+1) and the loop continues; failed rows retry hourly up to 5 attempts, then park (excluded from the queue so they can't crowd it) with a loud `parked` count logged every run for manual investigation.

**Response 200:** `{ "checked": n, "transferred": n, "adopted": n, "released": n, "held": n, "failed": n, "skipped": n, "errors": n, "parked": n }`. **Response 500:** `{ "error": "internal_error" }` (candidate/batch query failure — safe to retry next run).
...
### GET /api/admin/coaches/[id]/approve — PILOT
**Auth: `?secret=` query param — timing-safe comparison against server-only
`ADMIN_APPROVE_SECRET`. No admin session, no expiry.**

One-click coach approval, linked from the review-notification email (Email B,
founder inbox only). GET is deliberately side-effect-free — email link
scanners auto-fetch hrefs — and renders an HTML confirmation page whose
single button POSTs back to the same URL. Responses are minimal HTML pages,
not JSON.

- Wrong/missing secret, unknown coach, or never-submitted (draft) coach → 403
  "Invalid link" (secret is checked before any DB read)
- `stripe_onboarding_complete=false` → 409 (C-PAY-03 guard — never approve a
  coach without a payout destination)
- Already live → 200 "Already approved" (no form)

### POST /api/admin/coaches/[id]/approve — PILOT
Re-runs the full GET verification (never trusts the rendered page), then sets
`coach_profiles.is_profile_live=true` and sends the "profile live" email
(Email C) to the coach. Idempotent — a repeat POST returns the already-
approved page without re-writing or re-emailing. Uses the service-role client
(documented justification: single UPDATE of one row, gated on the secret;
the clicker has no Supabase session).

Pilot code isolation: logic lives in `src/lib/pilot/` — end of pilot deletes
that directory plus this route.
